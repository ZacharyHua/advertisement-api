...
littlec_parent_dir = os.path.dirname(python_dir)
littlec_connector_dir = os.path.join(littlec_parent_dir, "littlec-connector")
#  littlec_connector_dir = python_dir

grpc_out_dir=littlec_connector_dir + os.path.sep + "src" + os.path.sep + "main" + os.path.sep + "java"
proto_dir = python_dir + os.path.sep + "proto"
proto_java_dir = python_dir + os.path.sep + "src" + os.path.sep + "main" + os.path.sep + "java"
java_interface_dir = proto_java_dir + os.path.sep + "com" + os.path.sep + "cmcc" + os.path.sep + "littlec" + os.path.sep + "api"
...
def gen_interface_impl(inout_path, output_package, proto_file):
    ...
    proto_code_content = open(proto_file, 'r', encoding='utf-8').read()
    ...
    # write cmcc project information
    cmcc_project_info_w = cmcc_project_info.replace("COMMIT_DATE", commit_info["commit_date"])
    f.write(cmcc_project_info_w.encode(encoding='utf-8'))
    f.write("\n".encode(encoding='utf-8'))

    # write cmcc package information
    ...
    f.write("\n".encode(encoding='utf-8'))
    cmcc_class_info_w = cmcc_class_info.replace("SERVICE_NAME", service_info[0] + ".java").replace("CLASS_DESCRIPTION", service_info[1]).replace("COMMIT_USER", commit_info["commit_author"]).replace("COMMIT_TIME", commit_info["commit_time"])
    f.write(cmcc_class_info_w.encode(encoding='utf-8'))
    f.write("\n".encode(encoding='utf-8'))
    ...
    for method_name_w, method_info_w in method_info.items():
        method_request_name_w = method_info_w[1][0].lower() + method_info_w[1][1:]
        cmcc_method_info_w = cmcc_method_info.replace("METHOD_DESCRIPTION", method_info_w[3]).replace("REQUEST_TYPE", method_info_w[1]).replace("METHOD_RESPONSE", method_info_w[2]).replace("REQUEST_NAME", method_request_name_w)
        method_w = method_info_w[2] + " " + method_name_w + "(" + method_info_w[1] + " " + method_request_name_w + ");\n"
        f.write(("\n" + cmcc_method_info_w + "\n").encode(encoding='utf-8'))
        f.write((" " * 4 + method_w).encode(encoding='utf-8'))

    # end of interface
    f.write("\n}\n".encode(encoding='utf-8'))

    # write cmcc end information
    cmcc_end_info_w = cmcc_end_info.replace("COMMIT_DATE", commit_info["commit_date"]).replace("COMMIT_USER", commit_info["commit_author"])
    f.write("\n".encode(encoding='utf-8'))
    f.write(cmcc_end_info_w.encode(encoding='utf-8'))
    f.write("\n".encode(encoding='utf-8'))
...
def gen_interface(inout, proto_files):
    ...
    output_package = "com.study.multithread.api." + inout
